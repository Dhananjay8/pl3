...
#define DATA_SIZE 10
 ...
 int main(void)
 {
 	...
 	cl_mem inputA, inputB, output;		//OpenCL device buffer for scr and dest
 	
 	size_t global;		//OpenCL 1D variable
...
// retreive a list of platforms avaible
  ...
  // try to get a list of GPU device supported for available OpenCL platform
...
// create a context(i.e collection of GPU & CPU) with the GPU device
...
// copy the results from out of the output buffer to Host(CPU) variable
